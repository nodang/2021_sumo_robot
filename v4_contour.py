...
def contour():
        ...
        if contours:	#blue
            ...
            if all([len(approx) == 3, len(approx_) == 3]):
                ...
                Cv = np.array([aBB[1]*aB[0]-aBB[0]*aB[1],  aBB_[1]*aB_[0]-aBB_[0]*aB_[1]])
                ...
                if all([Cxy[0][1]/Cxy[0][0] != Cxy[1][1]/Cxy[1][0], \
                    ...
                    if all([0 < Cpoint[0] < 640, 0 < Cpoint[1] < 480]): 
                        ...
                        Cpoint = tuple(Cpoint)
                        cv2.circle(res, Cpoint, 2, (255, 0, 255), 10)
                    Mxy = np.array([[aB_[1]-aB[1], aB[0]-aB_[0]],[Cpoint[1]-RM[1], RM[0]-Cpoint[0]]])
                    Mv = np.array([aB_[1]*aB[0]-aB_[0]*aB[1], Cpoint[1]*RM[0]-Cpoint[0]*RM[1]])
                    ...
                    if all([Mxy[0][1]/Mxy[0][0] != Mxy[1][1]/Mxy[1][0], \
                        ...
                        l1 = dist(aB, aB_)
                        l2 = dist((X, 480), (X_, 480))
                        PixelDist = 2400/l2

                        SizeLine = l2/l1
                        realX = SizeLine*CamHeight/math.tan(24/180.0*math.pi)*PixelDist

                        print(l1,l2,SizeLine,realX)

                        #need to show on screen
                        if all([0 < Mpoint[0] < 640, 0 < Mpoint[1] < 480]):
                            Mpoint = int(Mpoint[0]), int(Mpoint[1])
                            Mpoint = tuple(Mpoint)
                            cv2.circle(res, Mpoint, 2, green, 10)
